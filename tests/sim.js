...
//   結局    wipe(被打光)/ crash(自己摔光)/ cap(兩邊都出滿 30 手)
...
function playGame(i, pair, timing) {
  ...
  return { i, seed, levels, plies, winner: st.winner, over: st.over, ending: endingOf(st), shots: st.shots.slice() };
}

// 結局:兩邊都出滿 MAX_SHOTS 就是 cap;否則看輸的那一邊三架是怎麼沒的
//(lost=true 是自己飛出紙外,by!=null 是被對方的線打到)。三架都是自己摔的 → crash。
function endingOf(st) {
  ...
  const loser = 1 - st.winner;
  const gone = st.planes.filter((p) => p.side === loser && !p.alive);
  return gone.length && gone.every((p) => p.lost) ? "crash" : "wipe";
}
